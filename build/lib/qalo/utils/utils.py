...
def plot2dstruct(data, fpath, elements, spc_size, unit_site):
    ...
    ax.set_yticks(np.arange(0, len(elements), step=1))
    plt.colorbar()
...
def calculate_pcf(poscar, elements, r_min=2.4, r_max=22.8, dr=0.1):
    structure = Poscar.from_file(poscar).structure
    n_bins = int(r_max / dr)
    n_pairs = int((1 + len(elements)) * len(elements) * 0.5) + 1
    
    pcf = np.zeros((n_bins, 1 + n_pairs))
    r_bins = np.arange(0, r_max + dr, dr)
    pcf[:, 0] = r_bins[:-1] + dr / 2
    
    pair_list = [(e1, e2) for i, e1 in enumerate(elements) for e2 in elements[i:]]
