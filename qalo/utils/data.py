...
def poscar2binvec(poscar, spcOriginal, spcDesign, site_positions, supercell, flatten=False):
    structure = Structure.from_file(poscar)
    ...
    for index, site in enumerate(structure):
        ...
        siteid = coord2idx(coords[0], coords[1], coords[2], site_positions, supercell)
...
def binvec2poscar(binvec, poscar, spc_size, elements, unit_site, alat):
    ...
    system['box_coord'] = []
    system['box_coord'].append([Nx * alat, 0.0, 0.0])
    system['box_coord'].append([0.0, Ny * alat, 0.0])
    system['box_coord'].append([0.0, 0.0, Nz * alat])
...
def raw2libffm(filepath, spcOriginal, spc_size, unit_site):
    Nx, Ny, Nz = spc_size[0], spc_size[1], spc_size[2]
    nsites = int(len(unit_site)) * Nx * Ny * Nz
    ...
    for structure in entries:
        ...
        if os.path.isdir(d) and structure.startswith("s-"):
            binvec, scaling_matrix = poscar2binvec(os.path.join(d, "POSCAR"), spcOriginal=spcOriginal, spcDesign=spc_size, site_positions=unit_site, supercell=spc_size, flatten=True)
...
def write_libffm_data(datapath, fmpath, spc_size, unit_site):
    ...
    for ds in datasets:
        ...
        libffm_data.extend(raw2libffm(d, spcOriginal, spc_size, unit_site))
...
def add_new_structure(energy, dfstack, fmpath, newpath, spc_size, unit_site):
    Nx, Ny, Nz = spc_size[0], spc_size[1], spc_size[2]
    nsites = int(len(unit_site)) * Nx * Ny * Nz
