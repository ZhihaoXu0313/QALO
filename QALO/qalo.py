...
class qalo(arguments):
    ...
    def run(self):
        ...
        libffm_data = write_libffm_data(datapath=self.dft_data_directory, fmpath=self.fm_directory)
        ffmModel = ffm()
        ...
        for i in tqdm(range(self.iterations)):
            ...
            with redirect(os.path.join(self.output_directory, "xlearn.log")):
                ffmModel.train(trainSet=os.path.join(self.fm_directory, "train_ffm.txt"),
                               ...
                               restart=True)
            composition = self.annealing_module(initial_composition=composition, 
                                                mix_circle=self.qa_mix_circle, annealer=self.qa_type)